...
public class SupplierController extends BaseController {
	...
	public void list(String supplierName,String agent,String phone,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (supplierName == null) supplierName = "";
		if (agent == null) agent = "";
		if (phone == null) phone = "";
		if(rows != 0)supplierService.setRows(rows);
		List<Supplier> supplierList = supplierService.querySupplier(supplierName, agent, phone, page);
	    /*计算总的页数和总的记录数*/
	    supplierService.queryTotalPageAndRecordNumber(supplierName, agent, phone);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String supplierName,String agent,String phone,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (supplierName == null) supplierName = "";
		if (agent == null) agent = "";
		if (phone == null) phone = "";
		List<Supplier> supplierList = supplierService.querySupplier(supplierName, agent, phone, currentPage);
	    /*计算总的页数和总的记录数*/
	    supplierService.queryTotalPageAndRecordNumber(supplierName, agent, phone);
	    /*获取到总的页码数目*/
	    int totalPage = supplierService.getTotalPage();
	...
	public void OutToExcel(String supplierName,String agent,String phone, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(supplierName == null) supplierName = "";
        if(agent == null) agent = "";
        if(phone == null) phone = "";
        List<Supplier> supplierList = supplierService.querySupplier(supplierName,agent,phone);
        ...
        for(int i=0;i<supplierList.size();i++) {
        	...
        	dataset.add(new String[]{supplier.getSupplierId() + "",supplier.getSupplierName(),supplier.getAgent(),supplier.getPhone(),supplier.getAddress()});
        }
        /*
