...
public class ProductController  extends BaseController {
    ...
    public void list(@ModelAttribute("productClassObj") ProductClass productClassObj,String productName,@ModelAttribute("supplierObj") Supplier supplierObj,String addTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if (page==null || page == 0) page = 1;
        if (productName == null) productName = "";
        if (addTime == null) addTime = "";
        if(rows != 0)productService.setRows(rows);
        List<Product> productList = productService.queryProduct(productClassObj, productName, supplierObj/*, addTime*/, page);
        /*计算总的页数和总的记录数*/
        productService.queryTotalPageAndRecordNumber(productClassObj, productName, supplierObj/*, addTime*/);
        /*获取到总的页码数目*/
    ...
    public String frontlist(@ModelAttribute("productClassObj") ProductClass productClassObj,String productName,@ModelAttribute("supplierObj") Supplier supplierObj,String addTime,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
        if (currentPage==null || currentPage == 0) currentPage = 1;
        if (productName == null) productName = "";
        if (addTime == null) addTime = "";
        List<Product> productList = productService.queryProduct(productClassObj, productName, supplierObj/*, addTime*/, currentPage);
        /*计算总的页数和总的记录数*/
        productService.queryTotalPageAndRecordNumber(productClassObj, productName, supplierObj/*, addTime*/);
        /*获取到总的页码数目*/
        int totalPage = productService.getTotalPage();
    ...
    public void OutToExcel(@ModelAttribute("productClassObj") ProductClass productClassObj,String productName,@ModelAttribute("supplierObj") Supplier supplierObj,String addTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(productName == null) productName = "";
        if(addTime == null) addTime = "";
        List<Product> productList = productService.queryProduct(productClassObj,productName,supplierObj/*,addTime*/);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Product信息记录";
        String[] headers = { "商品id","商品类别","商品名称","商品主图","商品价格","商品库存","供应商","发布时间"};
        List<String[]> dataset = new ArrayList<String[]>();
        for(int i=0;i<productList.size();i++) {
            Product product = productList.get(i);
            dataset.add(new String[]{product.getProductId() + "",product.getProductClassObj().getClassName(),product.getProductName(),product.getMainPhoto(),product.getPrice() + "",product.getProductNum() + "",product.getSupplierObj().getSupplierName()});
        }
        /*
